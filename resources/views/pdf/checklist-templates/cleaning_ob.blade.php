@php
    $shifts = array_values(array_filter((array) ($form['sections'] ?? []), 'is_array'));
...
@forelse($shifts as $pageIndex => $shift)
    @php
        $shiftId = (string) ($shift['id'] ?? '');
        $sections = array_values(array_filter((array) ($shift['sections'] ?? []), 'is_array'));
        $note = trim((string) ($areaNotes[$shiftId] ?? ''));
        $paths = $bucket($photoPaths[$shiftId] ?? []);
        $urls = $bucket($photoUrls[$shiftId] ?? []);
        $names = $bucket($photoNames[$shiftId] ?? []);
    ...
    <div class="form-page">
        ...
        <div class="control-row">
            ...
            <span class="fake-select">{{ $shift['name'] ?? $shift['title'] ?? $shiftId ?: '-' }}</span>
        </div>
            ...
            <tbody>
                @forelse($sections as $section)
                    <tr class="section-row">
                        <td colspan="3">{{ $section['title'] ?? $section['name'] ?? '-' }}</td>
                    </tr>
                    @foreach(array_values(array_filter((array) ($section['items'] ?? []), 'is_array')) as $idx => $item)
                ...
                @empty
                    <tr>
                        <td colspan="3" class="text-center">Belum ada item untuk shift ini.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="form-panel">
            <div class="form-panel-title">Keterangan {{ $shift['name'] ?? $shift['title'] ?? '' }}</div>
...
@empty
    <div class="form-page">
        ...
        <div class="note-box">Belum ada data shift.</div>
    </div>
@endforelse
