...
const moduleProfiles = [
  ...
  },
  {
    key: 'compress_pdf',
    label: 'Compress PDF',
    matches: ['gmisl/tools/compress', 'compress-pdf', 'compress pdf', 'compression pdf'],
...
const pageProfiles = [
  ...
  {
    key: 'compress_pdf_index',
    matches: ['gmisl/tools/compress-pdf', 'compress-pdf', 'compress_pdf'],
    suggestions: ['Bagaimana cara compress PDF?', 'Bagaimana batch compress?', 'Apa beda tingkat kompresi?'],
  },
];
...
const knowledgeTopics = [
  ...
  },
  {
    id: 'attendance_view',
    module: 'attendance',
    keywords: ['cara baca attendance log', 'cara lihat attendance', 'baca status attendance', 'langkah attendance'],
    summary: 'Untuk membaca dan memahami Attendance Log, ikuti langkah-langkah sistematis ini.',
    detail: [
      'Buka modul GMIHR dan pilih menu Attendance atau Time & Attendance.',
      'Dari submenu, pilih Attendance Log untuk melihat daftar kehadiran.',
      'Di halaman Attendance Log, gunakan filter untuk mempersempit data seperti tanggal, PIN karyawan, nama, atau department.',
      'Pastikan roster periode yang sedang dilihat sudah approved dan current di halaman Roster.',
      'Cek hasil attendance untuk setiap karyawan: On Time berarti tepat waktu, Terlambat berarti melewati batas, OFF berarti tidak ada jadwal atau hari libur.',
      'Jika hasil terasa aneh atau tidak sesuai, cek scan fingerprint mentah dan roster aktif pada tanggal yang sama.',
      'Perhatikan juga shift malam karena scan bisa lintas hari dan terbaca di tanggal berbeda.',
      'Jika ada anomali dan user adalah manager, bisa mengajukan koreksi attendance melalui alur yang disediakan.',
    ],
    suggestions: ['Kenapa attendance saya OFF?', 'Cara baca status attendance log'],
  },
  {
    id: 'leave_permission_create',
    module: 'leave_permission',
    keywords: ['cara buat leave permission', 'membuat leave permission', 'buat leave permission', 'langkah leave permission'],
    summary: 'Untuk membuat pengajuan Leave atau Permission dengan benar, ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul GMIHR dan pilih menu Attendance atau Time & Attendance.',
      'Dari submenu, pilih Leave & Permission lalu masuk ke halaman daftar atau buka tombol Create/Tambah pengajuan baru.',
      'Pilih tipe pengajuan yang sesuai dengan kebutuhan Anda, misalnya Cuti Tahunan, Cuti Sakit, atau Izin (sesuai opsi yang tersedia).',
      'Isi tanggal mulai dan tanggal selesai pengajuan dengan benar, pastikan tidak ada tabrakan dengan pengajuan sebelumnya.',
      'Isi alasan pengajuan dengan jelas dan ringkas sehingga approver mudah memahami maksud pengajuan Anda.',
      'Jika form menyediakan lampiran atau file pendukung (misalnya surat dokter untuk cuti sakit), upload file tersebut.',
      'Periksa kembali seluruh data sebelum submit, pastikan semua field terisi dan benar.',
      'Klik tombol Submit atau Simpan untuk mengirim pengajuan ke proses approval.',
      'Setelah submit, pengajuan akan masuk ke approver (biasanya atasan langsung atau manager). Cek status di halaman daftar leave permission.',
      'Jika status masih pending lama, hubungi approver yang bertanggung jawab atau admin untuk pengecekan.',
    ],
    suggestions: ['Siapa yang bisa approve leave permission?', 'Kenapa pengajuan saya belum approved?'],
  },
  {
    id: 'checklist_create',
    module: 'checklist',
    keywords: ['cara isi checklist', 'membuat checklist', 'buat checklist', 'langkah checklist'],
    summary: 'Untuk membuat dan mengisi Checklist berdasarkan template, ikuti langkah-langkah ini.',
    detail: [
      'Buka modul GMIIC dan pilih menu Checklist.',
      'Di halaman Checklist, klik tombol Create atau Tambah Checklist baru.',
      'Pilih template checklist yang sesuai dengan jenis pemeriksaan atau kebutuhan Anda dari dropdown yang tersedia.',
      'Sistem akan membuka form checklist dengan pertanyaan atau item yang sudah ditentukan dalam template.',
      'Jika template memiliki QRCode scan, pilih area atau section terlebih dahulu dari dropdown area yang sesuai.',
      'Scan QRCode area dengan barcode reader atau kamera, pastikan QRCode sesuai dengan area yang dipilih untuk menghindari kesalahan.',
      'Isi setiap pertanyaan atau item checklist sesuai hasil pemeriksaan di lapangan, gunakan checkbox, radio button, atau text input sesuai tipe pertanyaan.',
      'Tambahkan foto atau dokumen pendukung jika template atau proses pemeriksaan memerlukan bukti visual.',
      'Periksa kembali semua jawaban checklist sebelum submit untuk memastikan tidak ada yang terlewat.',
      'Klik tombol Submit atau Simpan untuk mengirim checklist ke proses review atau approval sesuai template yang digunakan.',
    ],
    suggestions: ['Bagaimana scan QRCode checklist?', 'Siapa yang bisa approve checklist?'],
  },
  {
    id: 'request_access_create',
    module: 'request_access',
    keywords: ['cara buat request access', 'membuat request access', 'buat request access', 'langkah request access'],
    summary: 'Untuk mengajukan akses modul bagi user baru atau perubahan akses, ikuti langkah-langkah ini.',
    detail: [
      'Buka modul GMISL, pilih menu Utility atau menu Request Access dari sidebar.',
      'Di halaman Request Access, klik tombol Create atau Tambah Request baru.',
      'Pilih jenis request: New User (jika membuat akses user baru) atau Existing User (jika mengubah akses user yang sudah ada).',
      'Jika jenis New User: isi data calon user seperti nama lengkap, email, department, dan position.',
      'Jika jenis Existing User: cari dan pilih user dari dropdown atau pencarian user yang sudah terdaftar.',
      'Pilih modul atau akses apa yang diminta, bisa satu atau lebih modul tergantung kebutuhan pekerjaan.',
      'Isi alasan request dengan jelas, misalnya "Promosi ke posisi baru", "Transfer department", atau "Perluas akses untuk project tertentu".',
      'Jika ada detail tambahan, tuliskan di bagian catatan atau keterangan yang disediakan.',
      'Periksa kembali semua data request sebelum submit, pastikan user dan modul sudah benar.',
      'Klik tombol Submit untuk mengirim request ke manager atau admin untuk ditinjau dan diproses.',
    ],
    suggestions: ['Siapa yang bisa approve request access?', 'Kenapa request access saya ditolak?'],
  },
  {
    id: 'check_inline_create',
    module: 'check_inline',
    keywords: ['cara buat check inline', 'membuat check inline', 'buat check inline', 'langkah check inline'],
    summary: 'Untuk membuat data Check Inline beserta gambar pendukung, ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul GMISL, pilih menu Utility atau menu Check Inline dari sidebar.',
      'Di halaman Check Inline, klik tombol Create atau Tambah data Check Inline baru.',
      'Isi nama atau deskripsi dari pemeriksaan inline yang akan dicatat di field "Nama" atau "Deskripsi".',
      'Pilih customer dari dropdown yang sesuai dengan purchase order atau project yang sedang dikerjakan.',
      'Isi nomor PO (Purchase Order) atau referensi order dari customer.',
      'Pilih batch atau lot nomor dari produksi atau barang yang sedang diperiksa.',
      'Isi jumlah atau quantity barang yang diperiksa pada field yang tersedia.',
      'Upload foto atau gambar hasil pemeriksaan untuk dokumentasi, pastikan gambar jelas dan menunjukkan hasil check yang dilakukan.',
      'Isi tanggal pemeriksaan dengan tanggal aktual saat check inline dilakukan.',
      'Periksa seluruh data yang sudah diisi, terutama customer, PO, batch, dan foto pendukung, lalu klik Submit atau Simpan.',
    ],
    suggestions: ['Siapa yang bisa update check inline?', 'Bagaimana lihat detail check inline?'],
  },
  {
    id: 'berita_acara_create',
    module: 'berita_acara',
    keywords: ['cara membuat berita acara', 'membuat berita acara', 'buat berita acara', 'langkah berita acara'],
    summary: 'Untuk membuat dokumen Berita Acara yang resmi, ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul GMISL, pilih menu Utility atau menu Berita Acara dari sidebar.',
      'Di halaman Berita Acara, klik tombol Create atau Tambah Berita Acara baru.',
      'Isi tanggal kejadian atau tanggal acara yang akan didokumentasikan dalam format yang diminta.',
      'Isi waktu kejadian atau acara dimulai dan berakhir dengan format jam:menit.',
      'Isi tempat atau lokasi terjadinya kejadian atau acara dengan deskripsi yang jelas.',
      'Pilih customer atau pihak terkait dari dropdown yang sesuai dengan kejadian.',
      'Pilih department yang terlibat atau bertanggung jawab dalam kejadian atau acara tersebut.',
      'Isi kronologi atau uraian kejadian secara detail dan sistematis, jelaskan apa yang terjadi dari awal hingga akhir.',
      'Jika ada data tambahan seperti nama PIC (person in charge), peserta, atau data lain, isi di bagian detail yang disediakan.',
      'Periksa seluruh dokumen sebelum submit, pastikan semua data lengkap dan akurat, lalu klik Submit atau Simpan dokumen.',
    ],
    suggestions: ['Bagaimana print berita acara?', 'Siapa yang bisa hapus berita acara?'],
  },
  {
    id: 'plugging_create',
    module: 'plugging',
    keywords: ['cara buat plugging', 'membuat plugging', 'buat plugging', 'langkah plugging'],
    summary: 'Untuk membuat dan mengajukan data Plugging, ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul GMIUM dan pilih menu Plugging atau klik submenu Plugging dari menu Utility atau Plugging.',
      'Di halaman Plugging, klik tombol Create atau Tambah data Plugging baru.',
      'Isi tanggal plugging dengan tanggal kegiatan plugging dilakukan di lapangan.',
      'Isi jam mulai dan jam selesai kegiatan plugging sesuai jadwal pelaksanaan aktual.',
      'Pilih customer atau perusahaan tempat plugging dilakukan dari dropdown yang tersedia.',
      'Pilih kendaraan atau equipment yang digunakan untuk kegiatan plugging.',
      'Isi suhu, tekanan, atau parameter teknis lain yang relevan dengan plugging sesuai standar yang berlaku.',
      'Isi lokasi atau detail lokasi plugging dilakukan untuk referensi dan tracking.',
      'Unggah dokumentasi atau foto kegiatan plugging jika form menyediakan field untuk lampiran.',
      'Periksa seluruh data yang sudah diisi, pastikan semuanya lengkap dan akurat, lalu klik Submit atau Simpan untuk mengirim ke proses approval.',
    ],
    suggestions: ['Bagaimana approval plugging?', 'Kenapa plugging saya belum approved?'],
  },
  {
    ...
    module: 'electricity',
    keywords: ['cara input meter listrik', 'membaca meter listrik', 'input electricity meter', 'langkah electricity'],
    summary: 'Untuk menginput dan mencatat pembacaan Meter Listrik (Standard atau HV), ikuti langkah-langkah ini.',
    detail: [
      'Buka modul GMIUM dan pilih menu Resource Monitoring atau menu Electricity dari sidebar.',
      'Pilih jenis meter yang akan dicatat: Standard Meter untuk meter reguler atau HV Meter untuk meter tegangan tinggi.',
      'Di halaman meter yang dipilih, klik tombol Create, Add, atau Input Pembacaan Baru.',
      'Pilih tanggal pembacaan meter sesuai tanggal aktual saat pembacaan dilakukan di lapangan.',
      'Isi nomor meter atau identifier meter yang sedang dibaca untuk memastikan data tercatat untuk meter yang benar.',
      'Isi nilai pembacaan meter dalam satuan yang sesuai (biasanya kWh untuk standard meter, kV untuk HV meter).',
      'Isi catatan atau keterangan tambahan jika ada kondisi khusus atau observasi saat pembacaan meter.',
      'Jika form menyediakan field untuk nama petugas atau foto bukti pembacaan, lengkapi informasi tersebut.',
      'Periksa kembali nomor meter dan nilai pembacaan sebelum submit untuk menghindari kesalahan input.',
      'Klik tombol Submit atau Simpan untuk menyimpan data pembacaan meter, dan cek kembali di daftar log untuk memastikan data sudah masuk.',
    ],
    suggestions: ['Bagaimana edit log meter listrik?', 'Bagaimana export data meter listrik?'],
  },
  {
    id: 'water_meter_input',
    module: 'water_meter',
    keywords: ['cara input water meter', 'membaca water meter', 'input meter air', 'langkah water meter'],
    summary: 'Untuk menginput dan mencatat pembacaan Water Meter, ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul GMIUM dan pilih menu Resource Monitoring atau menu Water Meter dari sidebar.',
      'Di halaman Water Meter, klik tombol Create, Add, atau Input Pembacaan Baru.',
      'Pilih atau tentukan tanggal pembacaan meter air sesuai tanggal aktual saat pembacaan dilakukan.',
      'Isi nomor meter atau identifier meter air yang sedang dibaca untuk tracking meter yang tepat.',
      'Isi nilai pembacaan meter air dalam satuan yang sesuai (biasanya m³ atau kubik meter untuk water meter).',
      'Jika form menyediakan lokasi atau area meter, pilih atau isi lokasi meter yang sedang dibaca.',
      'Isi catatan atau observasi tambahan jika ada kondisi khusus, seperti kebocoran atau anomali pembacaan.',
      'Jika ada field untuk nama petugas atau dokumentasi, isi sesuai data yang tersedia.',
      'Periksa kembali nomor meter dan nilai pembacaan sebelum submit untuk menghindari kesalahan pencatatan.',
      'Klik tombol Submit atau Simpan untuk menyimpan pembacaan, dan verifikasi di daftar log bahwa data sudah tercatat dengan benar.',
    ],
    suggestions: ['Bagaimana edit log water meter?', 'Bagaimana export water meter?'],
  },
  {
    id: 'visitor_form_create',
    module: 'visitor_form',
    keywords: ['cara buat visitor form', 'membuat visitor form', 'buat visitor form', 'langkah visitor form'],
    summary: 'Untuk membuat form kunjungan tamu (Visitor Form), ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul Leave Permit (Izin Keluar) atau cari menu Visitor Form dari sidebar.',
      'Di halaman Visitor Form, klik tombol Create atau Tambah Form Tamu Baru.',
      'Isi nama lengkap tamu dengan nama yang jelas dan sesuai dengan identitas tamu yang berkunjung.',
      'Isi nomor identitas atau nomor KTP/paspor tamu jika form memerlukan untuk keamanan dan pencatatan.',
      'Pilih tanggal kunjungan tamu dengan tanggal yang sesuai rencana kunjungan.',
      'Isi waktu kedatangan dan waktu perkiraan kepulangan tamu untuk estimasi durasi kunjungan.',
      'Isi tujuan kunjungan atau alasan tamu berkunjung dengan deskripsi yang jelas.',
      'Pilih atau isi nama pihak yang dikunjungi (internal employee atau department) yang akan bertemu dengan tamu.',
      'Jika form meminta, upload surat izin atau dokumen pendukung dari tamu atau pembawa tamu.',
      'Periksa seluruh data form, pastikan nama tamu, tanggal, waktu, dan tujuan sudah benar, lalu klik Submit untuk mengirim ke approval.',
    ],
    suggestions: ['Siapa yang bisa approve visitor form?', 'Kenapa visitor form saya belum approved?'],
  },
  {
    id: 'exit_permit_create',
    module: 'exit_permit',
    keywords: ['cara buat exit permit', 'membuat exit permit', 'buat exit permit', 'langkah exit permit'],
    summary: 'Untuk membuat Exit Permit atau surat izin keluar barang, ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul Leave Permit (Izin Keluar) atau cari menu Exit Permit dari sidebar.',
      'Di halaman Exit Permit, klik tombol Create atau Tambah Permit Baru.',
      'Isi tanggal permit atau tanggal direncanakan barang akan keluar dengan tanggal yang akurat.',
      'Isi deskripsi barang atau nama barang yang akan keluar dengan detail yang cukup untuk identifikasi.',
      'Isi jumlah atau quantity barang yang akan keluar sesuai kebutuhan.',
      'Pilih tujuan pengiriman atau lokasi barang akan dikirim dari dropdown atau tulis tujuan yang spesifik.',
      'Isi alasan pengiriman atau alasan barang perlu keluar, misalnya "Repair", "Maintenance", "Return to Supplier", dll.',
      'Jika form menyediakan field untuk nomor PO atau referensi order, isi referensi yang relevan.',
      'Upload dokumen pendukung seperti surat pesanan atau invoice jika diperlukan untuk verifikasi.',
      'Periksa semua data permit sebelum submit, pastikan barang, jumlah, tujuan, dan alasan sudah lengkap dan benar, lalu klik Submit untuk mengirim ke approval.',
    ],
    suggestions: ['Siapa yang bisa approve exit permit?', 'Bagaimana update status exit permit?'],
  },
  {
    ...
    module: 'master_data',
    keywords: ['cara buat master data', 'tambah master data', 'buat master data', 'langkah master data'],
    summary: 'Untuk menambah atau membuat data Master seperti Department, Employee, Position, Customer, dll, ikuti langkah ini.',
    detail: [
      'Buka modul Master Data dari menu sidebar atau Control Panel.',
      'Pilih jenis master data yang ingin ditambahkan: Department, Employee, Position, Customer, Vehicle Type, atau tipe master lainnya.',
      'Di halaman daftar master, klik tombol Create, Add, atau Tambah Data Baru.',
      'Isi field kode atau identifier dengan kode unik yang mudah diingat dan sesuai standar perusahaan (jika field tersedia).',
      'Isi nama atau deskripsi master data dengan nama yang jelas dan informatif.',
      'Isi field tambahan sesuai jenis master data, misalnya untuk Employee: nama lengkap, email, department, position; untuk Department: nama, kode, manager, dll.',
      'Jika master data memiliki relasi atau parent (misalnya subordinate untuk employee), pilih relasi yang sesuai dari dropdown.',
      'Isi field status atau flag lain yang relevan untuk mengaktifkan atau menonaktifkan data master tersebut.',
      'Periksa kembali semua field yang telah diisi, pastikan tidak ada typo dan data sudah sesuai dengan standar.',
      'Klik tombol Save, Create, atau Submit untuk menyimpan data master baru dan verifikasi di halaman daftar bahwa data sudah terdaftar.',
    ],
    suggestions: ['Bagaimana edit master data?', 'Siapa yang bisa mengelola master data?'],
  },
  {
    id: 'date_code_use',
    module: 'date_code',
    keywords: ['cara pakai date code', 'membaca date code', 'fungsi date code', 'langkah date code'],
    summary: 'Untuk menggunakan modul Date Code dalam membaca atau mengolah kode tanggal, ikuti langkah-langkah ini.',
    detail: [
      'Buka modul GMISL, pilih menu Utility atau cari menu Date Code dari sidebar.',
      'Di halaman Date Code, pahami format input yang dibutuhkan modul sesuai keterangan yang ditampilkan.',
      'Jika ada form, isi atau masukkan kode tanggal yang ingin dibaca atau dikonversi pada field yang tersedia.',
      'Pastikan format kode tanggal sesuai dengan format yang diminta modul, misalnya DDhMM atau DDMM untuk beberapa standar industri.',
      'Klik tombol Search, Convert, Generate, atau tombol proses lain yang tersedia untuk menjalankan pembacaan kode.',
      'Baca hasil pembacaan atau konversi yang ditampilkan oleh sistem sebagai penjelasan dari kode tanggal yang diinput.',
      'Jika hasil tidak sesuai ekspektasi, periksa kembali format kode yang diinput dan pastikan sudah sesuai dengan dokumen atau standar yang dipakai.',
      'Gunakan hasil pembacaan untuk referensi tanggal produksi, expiry date, atau interpretasi tanggal lain sesuai kebutuhan bisnis.',
      'Jika ada fitur export atau cetak hasil, gunakan tombol yang tersedia untuk menyimpan atau membagikan hasil pembacaan kode tanggal.',
    ],
    suggestions: ['Bagaimana membaca hasil date code?', 'Apa fungsi modul date code?'],
  },
  {
    id: 'control_panel_module_control',
    module: 'control_panel',
    keywords: ['cara mengubah module control', 'module control langkah', 'buat module control', 'akses modul user'],
    summary: 'Untuk mengelola akses modul user di Module Control, ikuti langkah-langkah berikut.',
    detail: [
      'Buka modul Control Panel dan pilih menu Module Control dari halaman control panel.',
      'Di halaman Module Control, akan tampil daftar user dan modul yang tersedia dalam aplikasi.',
      'Pilih user yang ingin diatur aksesnya dari daftar atau gunakan fitur cari/filter untuk menemukan user dengan cepat.',
      'Setelah user dipilih, tampil checklist atau list modul yang bisa diakses oleh user tersebut.',
      'Check (centang) modul yang ingin diberikan akses kepada user, uncheck (uncent) modul yang ingin dilarang.',
      'Pastikan kombinasi modul yang diberikan sesuai dengan job description dan tanggung jawab user.',
      'Jika ada opsi level atau tipe akses (misalnya View, Create, Edit, Delete), pilih level akses yang sesuai untuk setiap modul.',
      'Setelah selesai memilih modul dan level akses, klik tombol Save atau Update untuk menyimpan konfigurasi.',
      'Sistem akan menampilkan notifikasi jika perubahan berhasil tersimpan atau ada error yang perlu diperbaiki.',
      'User yang bersangkutan akan bisa mengakses modul baru setelah next login atau setelah cache/session di-refresh.',
    ],
    suggestions: ['Siapa yang bisa mengubah module control?', 'Bagaimana melihat log aktivitas?'],
  },
  {
    id: 'compress_pdf_overview',
    module: 'compress_pdf',
    keywords: ['compress pdf', 'kompresi pdf', 'cara compress pdf', 'cara kompresi pdf'],
    summary: 'Compress PDF adalah modul untuk mengkompresi ukuran file PDF dengan kualitas yang dapat disesuaikan.',
    detail: [
      'Modul ini memungkinkan user untuk mengupload file PDF dan mengkompresinya dengan berbagai tingkat kompresi.',
      'Ada tiga tingkat kompresi yang tersedia: Low (kualitas terbaik), Medium (seimbang), dan High (kompresi maksimal).',
      'File PDF yang sudah dikompres akan disimpan di server dan bisa diunduh kapan saja.',
      'Modul ini mendukung batch compress, sehingga user bisa mengupload dan mengkompresi beberapa file PDF sekaligus.',
      'Setiap job kompresi akan menyimpan informasi seperti ukuran file asli, ukuran terkompresi, dan persentase kompresi.',
      'User bisa melihat riwayat semua job kompresi yang sudah dilakukan dalam bentuk tabel atau daftar.',
      'Fitur download tersedia untuk file yang sudah selesai dikompres, dan user bisa menghapus job kompresi jika tidak diperlukan lagi.',
    ],
    suggestions: ['Bagaimana cara compress PDF?', 'Apa beda tingkat kompresi?', 'Bagaimana batch compress?'],
  },
  {
    id: 'compress_pdf_single',
    module: 'compress_pdf',
    keywords: ['cara compress pdf', 'langkah compress pdf', 'buat compress pdf', 'upload pdf compress'],
    summary: 'Untuk mengkompresi satu file PDF, ikuti langkah-langkah berikut dengan teliti.',
    detail: [
      'Buka modul GMISL, pilih menu Tools, kemudian pilih Compress PDF dari submenu.',
      'Di halaman Compress PDF, Anda akan melihat area upload file PDF dengan drag-and-drop atau click-to-select.',
      'Pilih file PDF yang ingin dikompres dengan mengklik area upload atau drag file langsung ke area tersebut.',
      'Setelah file dipilih, Anda akan melihat preview file yang dipilih di bagian Selected Files.',
      'Jika salah pilih file, klik tombol X di samping nama file untuk menghapusnya dari daftar.',
      'Setelah memilih file yang benar, pilih tingkat kompresi yang sesuai dengan kebutuhan: Low, Medium, atau High.',
      'Low compression memberikan kualitas terbaik dengan ukuran file lebih besar, cocok untuk dokumen penting yang perlu detail tinggi.',
      'Medium compression memberikan keseimbangan antara kualitas dan ukuran, cocok untuk sebagian besar kasus.',
      'High compression memberikan ukuran file terkecil dengan mengorbankan beberapa kualitas, cocok untuk dokumen yang tidak memerlukan detail tinggi.',
      'Setelah memilih file dan tingkat kompresi, klik tombol "Upload & Compress" untuk memulai proses kompresi.',
      'Proses kompresi akan berjalan di server, dan Anda akan melihat status job di bagian Compression History.',
      'Tunggu hingga status berubah menjadi "completed", lalu Anda bisa mengunduh file yang sudah dikompres dengan klik tombol download.',
    ],
    suggestions: ['Apa beda tingkat kompresi?', 'Bagaimana batch compress?', 'Bagaimana download file?'],
  },
  {
    id: 'compress_pdf_batch',
    module: 'compress_pdf',
    keywords: ['batch compress', 'batch compress pdf', 'multiple pdf', 'compress multiple pdf'],
    summary: 'Untuk mengkompresi beberapa file PDF sekaligus (batch), ikuti langkah-langkah ini.',
    detail: [
      'Buka modul GMISL, pilih menu Tools, kemudian pilih Compress PDF.',
      'Di halaman Compress PDF, area upload file sudah siap untuk menerima multiple files.',
      'Pilih beberapa file PDF sekaligus dengan tekan Ctrl (Windows) atau Cmd (Mac) sambil mengklik file-file yang ingin dipilih di file manager.',
      'Atau drag beberapa file PDF sekaligus ke area upload di halaman Compress PDF.',
      'Semua file yang dipilih akan muncul di bagian Selected Files dengan daftar lengkap jumlah file yang dipilih.',
      'Jika ada file yang tidak ingin dikompres, hapus dengan mengklik tombol X di samping nama file tersebut.',
      'Pilih tingkat kompresi yang sama untuk semua file: Low, Medium, atau High sesuai kebutuhan.',
      'Klik tombol "Upload & Compress" untuk memulai batch compression untuk semua file yang dipilih.',
      'Sistem akan memproses semua file secara berurutan atau paralel tergantung konfigurasi server.',
      'Lihat status setiap file di bagian Compression History - setiap file akan menampilkan status pending, processing, atau completed.',
      'Setelah semua file selesai dikompres (status completed), Anda bisa mengunduh masing-masing file dengan klik tombol download pada setiap baris.',
      'File-file yang sudah dikompres akan tersimpan di server dan bisa diakses kembali kapan saja dari tabel riwayat.',
    ],
    suggestions: ['Bagaimana single compress?', 'Apa beda tingkat kompresi?', 'Bagaimana download semua file?'],
  },
  {
    id: 'compress_pdf_understanding_levels',
    module: 'compress_pdf',
    keywords: ['tingkat kompresi', 'level kompresi', 'beda kompresi', 'low medium high'],
    summary: 'Perbedaan antara Low, Medium, dan High compression level dalam compress PDF.',
    detail: [
      'Low Compression (Best Quality):',
      '- Kompresi minimal, file hanya dikurangi ukuran sedikit',
      '- Kualitas visual PDF tetap sangat baik, cocok untuk dokumen penting',
      '- Ukuran file hasil kompresi masih relatif besar',
      '- Ideal untuk surat resmi, dokumen legal, atau gambar berkualitas tinggi',
      '',
      'Medium Compression (Balanced):',
      '- Kompresi seimbang antara kualitas dan ukuran',
      '- Pengurangan ukuran file cukup signifikan namun kualitas masih dapat diterima',
      '- Cocok untuk sebagian besar keperluan sehari-hari',
      '- Ideal untuk email attachment, file sharing, atau dokumen normal',
      '',
      'High Compression (Maximum Compression):',
      '- Kompresi maksimal untuk ukuran file sekecil mungkin',
      '- Ada penurunan kualitas visual yang terlihat, terutama untuk gambar atau grafis',
      '- Ukuran file hasil kompresi paling kecil di antara ketiga level',
      '- Ideal untuk dokumen teks sederhana, laporan internal, atau dokumen yang hanya perlu dibaca',
      '',
      'Tips memilih:',
      '- Jika file akan ditampilkan di presentasi atau dicetak → gunakan Low',
      '- Jika file untuk keperluan normal dan sharing → gunakan Medium',
      '- Jika file hanya untuk referensi atau file size sangat penting → gunakan High',
    ],
    suggestions: ['Langkah compress PDF', 'Bagaimana batch compress?'],
  },
];

const defaultTopicByModule = {
  attendance: 'attendance_view',
  leave_permission: 'leave_permission_create',
  overtime: 'overtime_create',
  roster: 'roster_upload',
  ticket: 'ticket_create',
  checklist: 'checklist_create',
  request_access: 'request_access_create',
  check_inline: 'check_inline_create',
  berita_acara: 'berita_acara_create',
  date_code: 'date_code_use',
  stock_card: 'stock_card_detail',
  plugging: 'plugging_create',
  electricity: 'electricity_input',
  water_meter: 'water_meter_input',
  utility_report: 'utility_report_overview',
  visitor_form: 'visitor_form_create',
  exit_permit: 'exit_permit_create',
  master_data: 'master_data_create',
  control_panel: 'control_panel_module_control',
  compress_pdf: 'compress_pdf_single',
...
const moduleActionFallbacks = {
  ...
  compress_pdf: [
    'Buka modul GMISL, pilih menu Tools, kemudian pilih Compress PDF.',
    'Upload satu atau lebih file PDF dengan drag-and-drop atau klik area upload.',
    'Pilih tingkat kompresi: Low (kualitas terbaik), Medium (seimbang), atau High (kompresi maksimal).',
    'Klik tombol "Upload & Compress" untuk memulai proses kompresi file.',
    'Pantau status kompresi di bagian Compression History - tunggu hingga status berubah menjadi completed.',
    'Setelah selesai, download file yang sudah dikompres dengan klik tombol download pada tabel.',
    'Lihat riwayat semua job kompresi di tabel bawah untuk referensi ukuran dan persentase kompresi.',
    'Hapus job kompresi yang tidak diperlukan lagi dengan klik tombol delete untuk membebaskan storage.',
  ],
};
