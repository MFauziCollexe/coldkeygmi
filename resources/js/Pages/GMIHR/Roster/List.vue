    ...
    <div class="space-y-6 p-4 md:p-6">
      ...
      <div class="rounded-lg border border-slate-700 bg-slate-800 p-4">
        ...
        <div v-else class="hidden overflow-auto lg:block">
          <table class="w-full min-w-[1420px] text-sm table-fixed">
            <thead class="text-slate-400 border-b border-slate-700">
              <tr>
                ...
                <th class="text-left py-2 pr-3 w-[156px]">Dept. Roster</th>
            ...
            <tbody>
              <tr v-for="batch in batches.data" :key="batch.id" class="border-b border-slate-700/60">
                ...
                <td class="py-2 pr-3 align-top truncate" :title="batch.department?.name || '-'">{{ batch.department?.name || '-' }}</td>
        ...
        <div v-if="batches.data.length" class="space-y-3 lg:hidden">
          ...
          >
            ...
            <div class="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
              ...
              <div>
                <div class="text-slate-400">Dept. Roster</div>
                <div>{{ batch.department?.name || '-' }}</div>
              </div>
              <div>
                <div class="text-slate-400">Uploader</div>
                <div>{{ batch.uploader?.name || '-' }}</div>
                <div class="text-xs text-slate-400">{{ batch.uploader?.department?.name || '-' }}</div>
              </div>
              <div>
