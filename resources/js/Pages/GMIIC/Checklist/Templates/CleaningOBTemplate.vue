        ...
        <div v-if="currentPhotos.length" class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          ...
          >
            ...
            >
              <img
                :src="photo.url"
                :alt="photo.name || `Foto cleaning ob ${index + 1}`"
  ...
  >
    <div class="w-full max-w-5xl rounded-xl bg-slate-900 p-4 shadow-2xl">
      <div class="mb-4 flex items-center justify-between gap-3">
        <div class="min-w-0">
          <h3 class="truncate text-lg font-semibold text-white">{{ previewPhoto.name || 'Foto Cleaning OB' }}</h3>
        </div>
      ...
      <div class="overflow-hidden rounded-lg border border-slate-700 bg-black">
        <img :src="previewPhoto.url" :alt="previewPhoto.name || 'Foto Cleaning OB'" class="max-h-[72vh] w-full object-contain" />
      </div>
      <div class="mt-4 flex justify-end">
        <a :href="previewPhoto.url" :download="previewPhoto.name || 'foto-cleaning-ob.jpg'" class="rounded bg-sky-600 px-4 py-2 text-sm font-semibold text-white hover:bg-sky-500">Download</a>
      </div>
    </div>
  </div>
</template>
...
function openPhotoPreview(photo, index) {
  ...
  previewPhoto.value = {
    ...photo,
    name: photo.name || `Foto Cleaning OB ${Number(index) + 1}`,
  }
}
