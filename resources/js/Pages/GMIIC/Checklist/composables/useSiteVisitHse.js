...
export function useSiteVisitHse(entry, { currentUser, showQrScanner }) {
    const isSiteVisitHse = computed(
        () => entry.value?.template_id === "site_visit_hse",
    );
    ...
    const currentSiteVisitHseSection = computed(() => {
        ...
        return (
            (entry.value.form.sections || []).find(
                (section) => section.id === areaId,
            ) || null
        );
    });
    ...
    const currentSiteVisitHseBarcode = computed(() => {
        if (!isSiteVisitHse.value || !entry.value) return "";
        return (
            entry.value.form.area_barcodes?.[siteVisitHseTargetKey.value] || ""
        );
    });
    ...
    const siteVisitHseNote = computed({
        get() {
            if (!isSiteVisitHse.value || !entry.value) return "";
            return (
                entry.value.form.area_notes?.[siteVisitHseTargetKey.value] || ""
            );
        },
        set(value) {
            if (!isSiteVisitHse.value || !entry.value) return;
    ...
    const siteVisitHseValidation = computed(() => {
        ...
        const hasRequiredNote =
            String(siteVisitHseNote.value || "").trim() !== "";
        return { allAnswersFilled, hasNoAnswer, hasRequiredNote };
    });

    const siteVisitHseCanApprove = computed(() => {
        if (!isSiteVisitHse.value || !entry.value) return false;
        ...
        if (
            ...
            return false;
        if (siteVisitHseApprovedAreas.value.includes(selectedArea))
            return false;
        // Enable approval based on the current area's answers and note requirement.
        // QR code scanning should not block per-area approval here.
        if (!siteVisitHseValidation.value.allAnswersFilled) return false;
        if (
            siteVisitHseValidation.value.hasNoAnswer &&
            !siteVisitHseValidation.value.hasRequiredNote
        )
            return false;
        return true;
    });
    ...
    function updateSiteVisitHseNote(value) {
        if (
            !entry.value ||
            !isSiteVisitHse.value ||
            !siteVisitHseTargetKey.value
        )
            return;
        entry.value.form.area_notes = {
            ...(entry.value.form.area_notes || {}),
    ...
    function cycleSiteVisitHseRowStatus(sectionId, itemId) {
        if (!entry.value || !isSiteVisitHse.value) return;
        if (
            siteVisitHseApprovedAreas.value.includes(
    ...
    function findOpenSiteVisitHseDraft(entries = []) {
        const today = toDateInputValue(new Date());
        const allEntries = Array.isArray(entries) ? entries : [];
        return (
            allEntries.find((c) => {
                if (!c || c.template_id !== "site_visit_hse") return false;
                const form = c.form || {};
                if (Boolean(form.approved)) return false;
                if (String(form.date_value || "") !== today) return false;
                const approved = Array.isArray(form.approved_areas)
                    ? form.approved_areas
                    : [];
                // if not all areas approved, resume this draft
                return approved.length < siteVisitHseAreaOptions.length;
            }) || null
        );
    }
    ...
    return {
        isSiteVisitHse,
        siteVisitHseApprovedAreas,
        currentSiteVisitHseSection,
        siteVisitHseTargetKey,
        currentSiteVisitHseBarcode,
        siteVisitHseNoteLabel,
        siteVisitHseNote,
        siteVisitHseValidation,
        canScanSiteVisitHse,
        siteVisitHseCanApprove,
        findOpenSiteVisitHseDraft,
        siteVisitHseAreaOptions,
        getSiteVisitHseAreaLabel,
        updateSiteVisitHseDate,
        updateSiteVisitHseArea,
        updateSiteVisitHseNote,
