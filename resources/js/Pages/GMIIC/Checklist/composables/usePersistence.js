...
function hydrateFireSafetyEntry(savedEntry) {
    if (savedEntry?.template_id !== "apar_smoke_detector_fire_alarm")
        return savedEntry;
    const cardType =
        String(savedEntry?.form?.card_type || "fire_alarm").trim() ||
        "fire_alarm";
    const locationId =
        String(savedEntry?.form?.location || "").trim() ||
    ...
    const nextEntry = {
        ...
        form: {
            ...
            card_type: cardType,
            location: locationId,
            location_records: { ...(savedEntry?.form?.location_records || {}) },
        },
    };
    if (!nextEntry.form.location_records[recordKey]) {
        nextEntry.form.location_records[recordKey] =
    ...
    nextEntry.form.rows = rebuildFireSafetyRows(
        cardType,
        loadedState.rows || [],
    );
    return nextEntry;
...
function hydrateSanitationEntry(savedEntry) {
    if (savedEntry?.template_id !== "non_warehouse_sanitation")
        return savedEntry;
    const legacyBackItems = new Set([
        "Ruang Mesin",
        "Ruang Kontrol",
        "Ruang Baterai",
    ]);
    const rowsByArea = { ...(savedEntry?.form?.rows_by_area || {}) };
    const legacyOutsideRows = Array.isArray(rowsByArea.area_luar_bangunan)
        ? rowsByArea.area_luar_bangunan
        : [];
    if (!Array.isArray(rowsByArea.lantai_1_depan) && legacyOutsideRows.length) {
        rowsByArea.lantai_1_depan = legacyOutsideRows.filter(
            (row) => !legacyBackItems.has(String(row?.name || "").trim()),
        );
    }
    if (
        !Array.isArray(rowsByArea.lantai_1_belakang) &&
        legacyOutsideRows.length
    ) {
        rowsByArea.lantai_1_belakang = legacyOutsideRows.filter((row) =>
            legacyBackItems.has(String(row?.name || "").trim()),
        );
    }
    const normalizedArea =
        String(savedEntry?.form?.area || "").trim() === "area_luar_bangunan"
            ? "lantai_1_depan"
...
function hydrateSaranaPrasaranaEntry(savedEntry) {
    ...
    return {
        ...
        form: {
            ...
            period,
            selected_area:
                String(savedEntry?.form?.selected_area || "").trim() ||
                savedEntry?.form?.sections?.[0]?.id ||
                "",
            area_scans_by_day: {
                ...(savedEntry?.form?.area_scans_by_day || {}),
            },
            approved_days_by_area: saranaPrasaranaAreaOptions.reduce(
                (result, area) => {
                    result[area.id] = Array.isArray(
                        savedEntry?.form?.approved_days_by_area?.[area.id],
                    )
                        ? savedEntry.form.approved_days_by_area[area.id]
                        : Array.isArray(savedEntry?.form?.approved_days) &&
                            area.id ===
                                (savedEntry?.form?.selected_area ||
                                    savedEntry?.form?.sections?.[0]?.id)
...
function hydratePatroliSecurityEntry(savedEntry) {
    ...
    const now = new Date();
    const nextDateValue =
        String(savedEntry?.form?.date_value || "").trim() ||
    ...
    return {
        ...
        form: {
            ...
            approved_areas: Array.isArray(savedEntry?.form?.approved_areas)
                ? savedEntry.form.approved_areas
                : [],
            area_barcodes: { ...(savedEntry?.form?.area_barcodes || {}) },
            ...
            area_scan_dates: { ...(savedEntry?.form?.area_scan_dates || {}) },
            document_no: savedEntry?.form?.document_no || "FRM.HSE.15.02",
            rev: savedEntry?.form?.rev || "00",
            effective_date:
                savedEntry?.form?.effective_date || "22 Desember 2025",
            page: savedEntry?.form?.page || "1 dari 1",
            sections: rebuildPatroliSecuritySections(
                savedEntry?.form?.sections || [],
            ),
        },
    };
    ...
    return {
        ...
        form: {
            ...
            effective_date:
                savedEntry?.form?.effective_date || "22 Desember 2025",
            page: savedEntry?.form?.page || "1 dari 1",
            sections: rebuildSiteVisitHseSections(
                savedEntry?.form?.sections || [],
            ),
        },
    };
...
function hydrateSiteVisitMaintenanceEntry(savedEntry) {
    ...
    const visitType =
        String(savedEntry?.form?.visit_type || "").trim() ||
        "maintenance_harian";
    ...
    const nextPeriodValue =
        String(savedEntry?.form?.period_value || "").trim() ||
        toWeekInputValue(now);
    const nextSelectedArea =
        String(savedEntry?.form?.selected_area || "").trim() ||
    ...
    return {
        ...
        form: {
            ...
            date_value: nextDateValue,
            date:
                visitType === "maintenance_harian"
                    ? savedEntry?.form?.date ||
                      formatDateInputDisplay(nextDateValue)
                    : "",
            period_value: nextPeriodValue,
            period:
                visitType === "maintenance_mingguan"
                    ? savedEntry?.form?.period ||
                      formatWeekDisplay(nextPeriodValue)
                    : "",
            sections: rebuildMaintenanceDailySections(
                savedEntry?.form?.sections || [],
            ),
...
function hydrateGensetRunningEntry(savedEntry) {
    ...
    const now = new Date();
    const legacySections = Array.isArray(savedEntry?.form?.sections)
        ? savedEntry.form.sections
        : [];
    const legacyActiveMonth =
        String(savedEntry?.form?.active_month || "").trim() || "jan";
    ...
    return {
        ...
        form: {
            ...
            period:
                String(savedEntry?.form?.period || "").trim() ||
                formatWeekDisplay(fallbackPeriodValue),
            selected_area:
                String(savedEntry?.form?.selected_area || "").trim() ||
                "genset",
            pic: savedEntry?.form?.pic || "User Login",
            date:
                String(savedEntry?.form?.date || "").trim() ||
                formatDateDisplay(now),
            document_no:
                String(savedEntry?.form?.document_no || "").trim() ||
                "DF-GMI-MTC-04",
            rev: String(savedEntry?.form?.rev || "").trim() || "00",
            effective_date:
                String(savedEntry?.form?.effective_date || "").trim() ||
                "22 Desember 2025",
            ...
            area_notes: { genset: "", ...(savedEntry?.form?.area_notes || {}) },
            rows: legacyRows,
        },
    };
}
...
function hydrateRunningGensetEntry(savedEntry) {
    ...
    return {
        ...
        form: {
            ...
            date:
                String(savedEntry?.form?.date || "").trim() ||
                formatDateInputDisplay(fallbackDateValue),
...
function hydrateKompresorHarianEntry(savedEntry) {
    ...
    const rows = rebuildKompresorDailyRows(
        period,
        savedEntry?.form?.rows || [],
    );
    const activeDay =
    ...
    return {
        ...
        form: {
            ...
            period,
            year:
                String(savedEntry?.form?.year || "").trim() ||
                year ||
...
function hydrateChargerBateraiEntry(savedEntry) {
    ...
    const period =
        ...
        toPeriodValue(new Date());
    const [year] = period.split("-");
    const rows = rebuildChargerBateraiRows(
        period,
        savedEntry?.form?.rows || [],
    );
    const activeDay =
    ...
    return {
        ...
        form: {
            ...
            year:
                ...
                String(new Date().getFullYear()),
            active_day: activeDay,
            serial_no: String(savedEntry?.form?.serial_no || ""),
            pic: savedEntry?.form?.pic || "User Login",
            document_no:
                String(savedEntry?.form?.document_no || "").trim() ||
                "DF-GMI-MTC-08",
...
function hydrateChecklistBateraiEntry(savedEntry) {
    ...
    const period =
        ...
        toPeriodValue(new Date());
    const [year] = period.split("-");
    const rows = rebuildChecklistBateraiRows(
        period,
        savedEntry?.form?.rows || [],
    );
    const activeDay =
    ...
    return {
        ...
        form: {
            ...
            year:
                ...
                String(new Date().getFullYear()),
            active_day: activeDay,
            battery_no: String(savedEntry?.form?.battery_no || ""),
            pic: savedEntry?.form?.pic || "User Login",
            document_no:
                String(savedEntry?.form?.document_no || "").trim() ||
                "DF-GMI-MTC-09",
...
function hydrateChecklistITEntry(savedEntry) {
    ...
    return {
        ...
        form: {
            ...savedEntry.form,
            week_value:
                String(savedEntry?.form?.week_value || "").trim() ||
                toWeekInputValue(new Date()),
            check_type:
                String(savedEntry?.form?.check_type || "").trim() || "hardware",
            check_items: { ...(savedEntry?.form?.check_items || {}) },
            pic: savedEntry?.form?.pic || "User Login",
            note: String(savedEntry?.form?.note || ""),
            approved: Boolean(savedEntry?.form?.approved),
        },
...
function hydrateChecklistEntry(savedEntry) {
    ...
    const hydrators = [
        hydrateFireSafetyEntry,
        hydrateSanitationEntry,
        hydrateSaranaPrasaranaEntry,
        hydratePatroliSecurityEntry,
        hydratePersonalHygieneEntry,
        hydrateSiteVisitHseEntry,
        hydrateSiteVisitMaintenanceEntry,
        hydrateGensetRunningEntry,
        hydrateRunningGensetEntry,
...
export function usePersistence({
    ...
    const page = usePage();
    const knownChecklistEntries = ref(
        Array.isArray(props.existingEntries) ? [...props.existingEntries] : [],
    );
    ...
    function upsertKnownChecklistEntry(savedEntry) {
        ...
        const idx = knownChecklistEntries.value.findIndex(
            (item) => item?.id === nextEntry.id,
        );
        if (idx === -1) {
            knownChecklistEntries.value = [
                nextEntry,
                ...knownChecklistEntries.value,
            ];
            return;
    ...
    function upsertKnownChecklistEntries(entries = []) {
        entries.forEach((e) => upsertKnownChecklistEntry(e));
    }
    ...
    function syncCurrentEntryUrl(targetEntry = entry.value) {
        ...
        window.history.replaceState(
            {},
            "",
            `/gmiic/checklist/create?${params.toString()}`,
        );
    }

    async function persistChecklistEntry(
        targetEntry = entry.value,
        options = {},
    ) {
        if (
            !targetEntry?.template_id ||
            !supportedTemplates.includes(targetEntry.template_id)
        )
            return targetEntry;
        ...
        if (
            !options.force &&
            signature !== "" &&
            signature === lastSavedEntrySignature
        )
    ...
    async function persistChecklistEntries(entries = []) {
        ...
        try {
            const response = await axios.post(
                "/gmiic/checklist/entries/bulk-save",
            ...
            saveState.value = "error";
            saveError.value =
                error?.response?.data?.message || "Gagal menyimpan checklist.";
            throw error;
        }
    }
    ...
    function syncSaveStateWithEntry() {
        if (
            !entry.value?.id ||
            !supportedTemplates.includes(entry.value?.template_id || "")
        ) {
        ...
        saveState.value =
            sig !== "" && sig === lastSavedEntrySignature ? "saved" : "dirty";
    }
    ...
    return {
        ...
        cloneChecklistEntry,
        buildEntrySignature,
        upsertKnownChecklistEntry,
        upsertKnownChecklistEntries,
        syncCurrentEntryUrl,
        persistChecklistEntry,
        persistChecklistEntries,
        syncSaveStateWithEntry,
        buildDigitalSignature,
        hydrateChecklistEntry,
    };
}
