...
export function useSiteVisitMaintenance(entry, { showQrScanner }) {
    const isSiteVisitMaintenance = computed(
        () => entry.value?.template_id === "site_visit_maintenance",
    );
    ...
    const maintenanceSections = computed(() => {
        ...
        const sections = Array.isArray(entry.value.form.sections)
            ? entry.value.form.sections
            : [];
        if (entry.value.form.visit_type !== "maintenance_harian")
            return sections;
        return sections.filter(
            (section) => String(section.area_id || "").trim() === selectedArea,
        );
    });

    const maintenanceRows = computed(() => {
        if (!isSiteVisitMaintenance.value || !entry.value) return [];
        return Array.isArray(entry.value.form.rows)
    ...
    const currentMaintenanceBarcode = computed(() => {
        if (!isSiteVisitMaintenance.value || !entry.value) return "";
        return (
            entry.value.form.area_barcodes?.[maintenanceScanTargetKey.value] ||
            ""
        );
    });
    ...
    const maintenanceNote = computed({
        get() {
            if (!isSiteVisitMaintenance.value || !entry.value) return "";
            return (
                entry.value.form.area_notes?.[maintenanceScanTargetKey.value] ||
                ""
            );
        },
        set(value) {
    ...
    const canScanMaintenance = computed(() => {
        ...
        return (
            ...
            (!maintenanceValidation.value.hasNoAnswer ||
                maintenanceValidation.value.hasRequiredNote) &&
    ...
    const currentMaintenancePhotos = computed(() => {
        if (!isSiteVisitMaintenance.value || !entry.value) return [];
        const paths = normalizeBucket(
            entry.value.form.area_photo_paths?.[maintenanceScanTargetKey.value],
        );
        const urls = normalizeBucket(
            entry.value.form.area_photo_urls?.[maintenanceScanTargetKey.value],
        );
        const names = normalizeBucket(
            entry.value.form.area_photo_names?.[maintenanceScanTargetKey.value],
        );
    ...
    function updateMaintenanceVisitType(value) {
        ...
        entry.value.form.selected_area =
            visitType === "maintenance_harian"
                ...
                : "lantai_1_area_belakang";
        entry.value.form.date =
            visitType === "maintenance_harian"
                ? formatDateInputDisplay(
                      entry.value.form.date_value ||
        ...
        entry.value.form.period =
            visitType === "maintenance_mingguan"
                ? formatWeekDisplay(
                      entry.value.form.period_value ||
                          toWeekInputValue(new Date()),
                  )
                : "";
        entry.value.form.sections = rebuildMaintenanceDailySections(
            entry.value.form.sections || [],
        );
        entry.value.form.rows = rebuildMaintenanceWeeklyRows(
            entry.value.form.rows || [],
        );
    }
        ...
        if (entry.value.form.visit_type === "maintenance_mingguan") {
            entry.value.form.rows = (entry.value.form.rows || []).map((row) =>
                row.id === payload.rowId
                    ? { ...row, status: getNextMaintenanceStatus(row.status) }
                    : row,
            );
            return;
        }
        entry.value.form.sections = (entry.value.form.sections || []).map(
            (section) =>
                    ...
                    ? {
                          ...
                          items: (section.items || []).map((item) =>
                              item.id === payload.rowId
                                  ? {
                                        ...item,
                                        status: getNextMaintenanceStatus(
                                            item.status,
                                        ),
                                    }
                                  : item,
    ...
    function hasPendingMaintenanceAreas(savedEntry) {
        const pendingAreas = Array.isArray(savedEntry?.form?.approved_areas)
            ? savedEntry.form.approved_areas
            : [];
        const allAreas = maintenanceDailyAreaOptions.map((area) => area.id);
        return !allAreas.every((areaId) => pendingAreas.includes(areaId));
    }

    function findOpenSiteVisitMaintenanceDraft(entries = []) {
        const currentDate = toDateInputValue(new Date());
        return (
            (Array.isArray(entries) ? entries : []).find((c) => {
                if (c?.template_id !== "site_visit_maintenance") return false;
                const visitType = String(c?.form?.visit_type || "").trim();
                if (visitType === "maintenance_harian") {
                    return (
                        String(c?.form?.date_value || "").trim() ===
                            currentDate &&
                        !Boolean(c?.form?.approved) &&
                        hasPendingMaintenanceAreas(c)
                    );
                }
                if (visitType === "maintenance_mingguan") {
                    return (
                        String(c?.form?.period_value || "").trim() ===
                            toWeekInputValue(new Date()) &&
                        !Boolean(c?.form?.approved)
                    );
                }
    ...
    return {
        ...
        currentMaintenanceBarcode,
        maintenanceNoteLabel,
        maintenanceNote,
        maintenanceValidation,
        canScanMaintenance,
        currentMaintenancePhotos,
        maintenancePhotoUploading,
        maintenancePhotoError,
        maintenanceVisitTypeOptions,
        maintenanceDailyAreaOptions,
        getMaintenanceDailyAreaLabel,
        updateMaintenanceVisitType,
        updateMaintenanceVisitDate,
        updateMaintenanceVisitPeriod,
        updateMaintenanceVisitArea,
        cycleMaintenanceRowStatus,
        updateMaintenanceNote,
        findOpenSiteVisitMaintenanceDraft,
        normalizeBucket,
    };
}
