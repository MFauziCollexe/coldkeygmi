...
import {
    cleaningOBShiftOptions,
    getCleaningOBShiftLabel,
    rebuildCleaningOBSections,
    formatDateInputDisplay,
    toDateInputValue,
} from "../checklistConfig";

export function useCleaningOB(entry, { currentUser, showQrScanner }) {
    const isCleaningOB = computed(
        () => entry.value?.template_id === "jadwal_cleaning_ob",
    );
    ...
    const currentCleaningOBPhotos = computed(() => {
        ...
        if (!targetKey) return [];
        const paths = normalizeCleaningOBPhotoBucket(
            entry.value.form.area_photo_paths?.[targetKey],
        );
        const urls = normalizeCleaningOBPhotoBucket(
            entry.value.form.area_photo_urls?.[targetKey],
        );
        const names = normalizeCleaningOBPhotoBucket(
    ...
    const cleaningOBApprovedAreas = computed(() => {
        if (!isCleaningOB.value || !entry.value) return [];
        return Array.isArray(entry.value.form.approved_areas)
    ...
    const currentCleaningOBSections = computed(() => {
        ...
        const shift = (entry.value.form.sections || []).find(
            (s) => s.id === shiftId,
        );
    ...
    const isCleaningOBFromToday = computed(() => {
        if (!isCleaningOB.value || !entry.value) return false;
        return (
            String(entry.value.form.date_value || "").trim() === todayDateValue
        );
    });

    const isCleaningOBEditable = computed(
        () => isCleaningOB.value && isCleaningOBFromToday.value,
    );

    const currentCleaningOBBarcode = computed(() => {
        if (!isCleaningOB.value || !entry.value) return "";
        return (
            entry.value.form.area_barcodes?.[cleaningOBTargetKey.value] || ""
        );
    });
    ...
    const cleaningOBNote = computed({
        get() {
            if (!isCleaningOB.value || !entry.value) return "";
            return (
                entry.value.form.area_notes?.[cleaningOBTargetKey.value] || ""
            );
        },
        set(value) {
            if (!isCleaningOB.value || !entry.value) return;
            entry.value.form.area_notes = {
                ...(entry.value.form.area_notes || {}),
                [cleaningOBTargetKey.value]: value,
            };
        },
    });

    const cleaningOBValidation = computed(() => {
        if (!isCleaningOB.value || !entry.value)
            return {
                allAnswersFilled: false,
                hasNoAnswer: false,
                hasRequiredNote: false,
            };
        const activeItems = currentCleaningOBSections.value.flatMap(
        ...
        const hasRequiredNote =
            String(cleaningOBNote.value || "").trim() !== "";
        return { allAnswersFilled, hasNoAnswer, hasRequiredNote };
    });

    const canScanCleaningOB = computed(() => {
        if (!isCleaningOB.value || !entry.value || !showQrScanner.value)
            return false;
        ...
        return (
            ...
            (!cleaningOBValidation.value.hasNoAnswer ||
                cleaningOBValidation.value.hasRequiredNote) &&
    ...
    function findOpenCleaningOBDraft(entries = []) {
        ...
        return (
            (Array.isArray(entries) ? entries : []).find(
                (c) =>
                    c?.template_id === "jadwal_cleaning_ob" &&
                    !Boolean(c?.form?.approved) &&
                    String(c?.form?.date_value || "").trim() === todayValue,
            ) || null
        );
    }
    ...
    function updateCleaningOBNote(value) {
        ...
        entry.value.form.area_notes = {
            ...(entry.value.form.area_notes || {}),
            [cleaningOBTargetKey.value]: String(value || ""),
        };
    }
    ...
    function cycleCleaningOBRowStatus(sectionId, itemId) {
        if (!entry.value || !isCleaningOB.value) return;
        if (
            cleaningOBApprovedAreas.value.includes(
                String(entry.value.form.selected_shift || "").trim(),
            )
        )
            return;
    ...
    return {
        isCleaningOB,
        cleaningOBApprovedAreas,
        currentCleaningOBSections,
        cleaningOBTargetKey,
        currentCleaningOBBarcode,
        cleaningOBNoteLabel,
        cleaningOBNote,
        cleaningOBValidation,
        canScanCleaningOB,
        cleaningOBShiftOptions,
        getCleaningOBShiftLabel,
        isCleaningOBEditable,
        findOpenCleaningOBDraft,
        updateCleaningOBDate,
        updateCleaningOBShift,
        updateCleaningOBNote,
        cycleCleaningOBRowStatus,
