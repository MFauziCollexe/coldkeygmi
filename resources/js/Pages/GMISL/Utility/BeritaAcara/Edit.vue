<template>
  <AppLayout>
    <div class="max-w-5xl space-y-4 p-4 md:p-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 class="text-2xl font-bold">Edit Berita Acara</h2>
          <p class="text-slate-400 text-sm">Ubah data Berita Acara.</p>
        </div>
        <Link :href="`/gmisl/utility/berita-acara/${item.id}`" class="text-indigo-400 hover:underline text-sm">← Back to Detail</Link>
      </div>
      ...
      <div class="rounded-lg border border-slate-700 bg-slate-800 p-4">
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div class="flex justify-end">
            <button type="submit" class="w-full rounded bg-indigo-600 px-4 py-2 text-white md:w-auto" :disabled="form.processing">
              Simpan Perubahan
            </button>
          </div>
        </form>
...
const form = useForm({
  incident_date: String(props.item?.event_date || ''),
  incident_place: String(props.item?.event_location || ''),
  incident_time: String(props.item?.incident_time || props.item?.start_time || '').slice(0, 5),
  customer_id: String(props.item?.customer_id || ''),
  vehicle_no: String(props.item?.vehicle_no || ''),
  department_id: String(props.item?.department_id || ''),
  chronology: String(props.item?.chronology || ''),
});

function submit() {
  form.put(`/gmisl/utility/berita-acara/${props.item.id}`);
}
</script>
