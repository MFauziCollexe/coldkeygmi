    ...
    <div class="p-4 md:p-6">
      ...
      <form @submit.prevent="submit">
        <div class="max-w-4xl rounded bg-slate-800 p-4 md:p-6">
          ...
          <!-- NIK -->
          <div class="mb-4">
            <label class="block text-sm font-medium mb-2">Nama Fingerprint</label>
            <input
              :value="employeeData.name || ''"
              type="text"
              readonly
              class="w-full cursor-not-allowed rounded border bg-slate-200 px-4 py-2 text-slate-700 dark:bg-slate-700 dark:text-slate-200"
            />
            <p class="mt-1 text-xs text-slate-400">
              Nama asli dari data fingerprint. Field ini tidak mengubah nama mentah dari mesin fingerprint.
            </p>
          </div>

          <div class="mb-4">
            <label class="block text-sm font-medium mb-2">Nama Alias</label>
            <input
              v-model="form.alias_name"
              type="text"
              class="w-full px-4 py-2 border rounded bg-white dark:bg-gray-700"
              placeholder="Nama yang ditampilkan di attendance log"
            />
            <p v-if="errors.alias_name" class="text-red-500 text-sm mt-1">{{ errors.alias_name }}</p>
          </div>
