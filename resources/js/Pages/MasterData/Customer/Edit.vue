      ...
      <form @submit.prevent="submit" class="bg-slate-800 rounded p-6 space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <img v-if="customer.logo_image_url" :src="customer.logo_image_url" class="mt-2 w-20 h-20 object-cover rounded border border-slate-600" />
          </div>
...
function submit() {
  form.transform(data => ({ ...data, _method: 'put' })).post(`/master-data/customer/${customer.id}`, {
    forceFormData: true,
  });
}
</script>
